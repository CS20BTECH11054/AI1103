...
\end{align}
But we know that there is no probability for the random variable X to satisfy $X \leq 0$,So, from \eqref{Eq:5}, we can write,
\begin{align}
F_X\brak{0} = P\brak{X \leq 0} = 0 \label{Eq:8}
\end{align}
Substituting \eqref{Eq:8} and \eqref{Eq:5} in \eqref{Eq:4}, we get
\begin{align}
P\brak{X\leq x} = F_X\brak{x} = 1 - e^{-x} \label{Eq:12}
\end{align}
